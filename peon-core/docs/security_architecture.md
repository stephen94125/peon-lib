...
> Peon operates on a strict **"prove it before you touch it"** principle.
...
Peon was built to close that gap at the architecture level, not the prompt level.
...
A dual-enforcer layer sits behind Layer 1, evaluating `(subject, action, resource)` triples using Casbin `file_permissions.txt` and `user_permissions.csv` settings. This acts as an iron-clad "deny override" blacklist, blocking specific paths or users globally even if a skill whitelists them.
